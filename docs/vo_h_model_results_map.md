...
| `Blok A` | symptom fidelity voči seedu | `VO2` | `H4` | `symptom_error_mean` = priemerná absolútna chyba `A1`–`A9`; doplnkovo `severity_error`, `impact_error`, deskriptívne `S1`, `S2` | deskriptíva, transcript-level model pre `symptom_error_mean`, `LMM` pre priame error skóre, doplnkovo `CLMM` pre `S1`, `S2` | `3.2`, `3.3`, `3.5.4` | Tu nejde len o to, či rozhovor znie dobre, ale či zodpovedá zamýšľanému klinickému profilu seedu; jadro je rozdiel v priemernej chybe, nie voľný dojem. |
...
| `Blok B` | efekt `profile` na symptom fidelity a red flags | `VO4` | `H8` + bez samostatnej hypotézy pre zvyšné outcome-y | `symptom_error_mean`, `severity_error`, `impact_error`, `defect_index` | `LMM` a transcript-level model; doplnkovo `CLMM` pre `S1`, `S2` | `3.6.3` | `H8` pokrýva jadro transcript-level symptom fidelity; ostatné outcome-y ostávajú inferenčnou odpoveďou na `VO4`, nie samostatnými smerovými hypotézami. |
| `Blok C` | interakcia `guardrail × profile` | `VO5` | `H9` | index klinickej vierohodnosti, index defektov, doplnkovo `symptom_error_mean`, `severity_error`, `impact_error` | interakčné členy v `LMM`, transcript-level modeli a doplnkových ordinálnych modeloch | `3.6.4` | Tu sa pýtam, či `guardrail` pomáha rovnako vo všetkých profiloch, alebo výraznejšie pri niektorých štýloch odpovedania. |
