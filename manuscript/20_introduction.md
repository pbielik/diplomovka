...
>
> Poznámka: citekeys `who2025depression`, `chaby2022embodiedvirtualpatients`, `li2024curefun`, `wang2024patientpsi`, `lee2025adaptivevp` a `kim2025mindvoyager` sú momentálne placeholdery. Zdroje sú evidované ako gaps F–I v `docs/literature/gaps_and_missing_sources.md` a čakajú na import do Zotera (backlog položka `B22`).
...
Existujúce práce ukazujú, že kvalita simulovaných pacientov nezávisí iba od samotnej jazykovej plynulosti modelu, ale vo veľkej miere od toho, či je simulácia klinicky ukotvená a štruktúrovaná. [@li2024curefun] opisuje framework CureFun, ktorý využíva explicitné promptingové postupy a chain-of-thought usmernenie na riadenie správania modelu podľa vopred určenej procedúry; v prezentovaných evaluáciách uvádza autentickejšie a profesionálnejšie dialógové toky než pri iných LLM chatbotových riešeniach. [@wang2024patientpsi] v práci PATIENT-ψ ukazujú, že integrácia CBT kognitívnych modelov do simulácie pacienta vedie k vyššej vernosti simulácie; podľa expertov bol PATIENT-ψ bližší reálnym pacientskym interakciám než GPT-4 baseline. [@lee2025adaptivevp] v rámci Adaptive-VP navyše ukázali, že klinicky ukotvené scenáre a dynamické prispôsobovanie odpovedí dokážu viesť k prirodzenejším a realistickejším interakciám, čo potvrdili aj expertne hodnotiace sestry.
