...
Výskum bol navrhnutý ako **faktorový experiment v rámci odborného hodnotenia transkriptov**, v ktorom boli systematicky obmieňané dve nezávislé premenné. Prvou bola **prítomnosť štruktúrujúceho klinického usmernenia** s dvoma úrovňami: bez usmernenia a so štruktúrujúcim klinickým usmernením. Druhou bol **štýl odpovedania simulovaného pacienta** s tromi úrovňami: P1, P2 a P3. Kombináciou týchto faktorov vzniklo šesť finálnych experimentálnych variantov. Každý variant bol aplikovaný na 12 vopred pripravených a klinicky ukotvených seed scenárov, čím vznikol súbor **72 transkriptov** určených na odborné hodnotenie. Klinická variabilita medzi rozhovormi bola teda nesená najmä seed scenármi, zatiaľ čo experimentálna variabilita bola cielene reprezentovaná dvoma manipulovanými faktormi generovania.
...
Finálny experimentálny preset pozostával zo šiestich variantov definovaných ako G0/G1 × R1/R2/R3 v dátovom exporte, kde G0/G1 označovalo neprítomnosť alebo prítomnosť štruktúrujúceho klinického usmernenia a R1/R2/R3 predstavovalo tri úrovne štýlu odpovedania simulovaného pacienta. Aby sa exportné označenie profilu nebilo s defektovými položkami R1 až R5, v hlavnom texte práce tento faktor autorizačne prepisujeme ako P1/P2/P3. V psychologickom jazyku teda pracujeme s týmto mapovaním:

- G0 = bez štruktúrujúceho klinického usmernenia,
- G1 = so štruktúrujúcim klinickým usmernením,
- R1 / P1 = zdržanlivý štýl odpovedania simulovaného pacienta,
- R2 / P2 = vyvážený štýl odpovedania simulovaného pacienta,
- R3 / P3 = rozvinutý štýl odpovedania simulovaného pacienta.
...
**Transcript-level symptom metadata A1 až A9** nepredstavujú dodatočné ľudské ratingy od ratera. Ide o štruktúrované PHQ-9 symptom score polia importované zo Sim Lab / Chat Lab bundle, teda o self-report vrstvu pripojenú k danému simulovanému participantovi a transkriptu. Položky A1 až A9 zodpovedajú deviatim obsahovým oblastiam depresívnej symptomatiky zavedeným v Úvode (1.1) a previazaným s logikou PHQ-9 [@kroenke2001phq9]. V analýze preto slúžia prednostne ako transcript-level symptom profil a ako podklad pre porovnanie s vopred definovanými seed anchor hodnotami, nie ako psychometrická súčasť expertného ratingového nástroja.
...
Položky G1 až G5, S1, S2, položky R1 až R5 a položka zachytávajúca istotu odhadu pôvodu boli koncipované ako **5-bodové škály** a v analýze sú chápané ako ordinálne premenné. Naproti tomu A1 až A9 nie sú súčasťou ľudského ratingového formulára, ale importované transcript-level PHQ-9 symptom skóre na **škále 0–3**. Toto rozlíšenie je metodicky dôležité: expertná evaluácia a transcript-level symptom metadata sa v práci používajú spoločne, ale nepredstavujú jednu a tú istú meraciu vrstvu.

Takéto členenie dátovej štruktúry umožnilo oddeliť jadro expertného hodnotenia rozhovoru od transcript-level symptom profilu a od vrstvy kontroly kvality. Zároveň tým vznikol priamy most medzi teoretickými konštruktmi v Úvode (klinická vierohodnosť, prirodzenosť, konzistentnosť, symptomatická primeranosť, varovné signály, tréningová použiteľnosť — časť 1.5) a empirickými premennými použitými v analýze.
...
Popri neskoršej analýze vnútornej konzistencie a zhody medzi hodnotiteľmi bola do pracovného workflowu zaradená aj **predbežná expertná obsahová kontrola** autorského ratingového dotazníka a seed scenárov. V tejto vrstve experti neposudzovali hotové transkripty, ale samotné položky nástroja a klinicky ukotvené seed scenáre ešte pred ich finálnym použitím v hlavnom hodnotení. Pri položkách sa sledovala ich relevancia, zrozumiteľnosť a potrebnosť pre finálny nástroj; pri seed scenároch klinická plausibilita, primeranosť symptomatických anchorov, jasnosť, dostatok informácií pre generovanie rozhovoru, nestereotypnosť a použiteľnosť pre výskumný alebo výučbový kontext.

V aktuálnom stave táto vrstva zahŕňala **šesť expertov**. Každý expert hodnotil 12 položiek ratingového nástroja na troch kritériách (relevancia, zrozumiteľnosť, potrebnosť) a 12 seed scenárov na siedmich hodnotiacich dimenziách, doplnených odhadom cieľovej závažnosti a otvoreným komentárom. Táto vrstva preto v práci nebude interpretovaná ako plnohodnotná psychometrická validácia ani ako formálny Delphi proces. Jej funkciou je zachytiť zjavne problematické formulácie, nejednoznačné položky alebo klinicky sporné seed nastavenia a zdokumentovať odporúčané redakčné úpravy pred hlavným hodnotením transkriptov [@polit2007cvi; @hasson2000delphi; @boateng2018scaledevelopment; @rutherfordhemming2015simulationcvi]. Pri položkách dotazníka možno pomocne uviesť aj item-level proporcie hodnotení `3-4`, tie však budú vzhľadom na veľkosť panelu interpretované opatrne a vždy spolu s kvalitatívnymi komentármi expertov [@terwee2018COSMINMethodologyEvaluating; @polit2007cvi].
...
**Štýl odpovedania simulovaného pacienta** bol operacionalizovaný ako trojúrovňová kategoriálna nezávislá premenná. V exporte je uložený v poli `profile` s kódmi R1 až R3; v texte práce používame autorizačné označenie P1 až P3:
...
Primárne závislé premenné tvorili položky G1 až G5 z prvého bloku ratingového nástroja. Na **deskriptívnej úrovni** budú tieto položky reportované ako ordinálne ukazovatele prostredníctvom frekvencií, mediánov a interkvartilových rozpätí. Na **inferenčnej úrovni** budú položky G2 a G5 vystupovať ako samostatné ordinálne outcome-y modelované pomocou ordinal mixed models; položky G1, G3 a G4 budú slúžiť prednostne ako obsahová opora kompozitu klinickej vierohodnosti a len sekundárne ako položkové spresnenie hlavného výsledku.

Okrem jednotlivých položiek bol vytvorený kompozitný ukazovateľ **indexu klinickej vierohodnosti**, vypočítaný ako priemer položiek G1, G3 a G4, keďže tieto tri položky podľa projektovej logiky a obsahovej validity konceptuálne najtesnejšie reprezentujú jadro klinickej vierohodnosti simulovaného interview.

Index klinickej vierohodnosti bol vypočítaný ako: (G1 + G3 + G4) / 3.

Keďže ide o kompozit s približne intervalovým charakterom, index klinickej vierohodnosti bude v tabuľkách reportovaný primárne pomocou priemeru a smerodajnej odchýlky a doplnkovo možno uviesť aj medián a interkvartilové rozpätie. V inferenčnej analýze vystupuje ako hlavný outcome pre lineárne zmiešané modely. Položka G2 (prirodzenosť jazyka) je analyzovaná samostatne ako ordinálny ukazovateľ jazykovej a interakčnej kvality. Položka G5 (tréningová použiteľnosť) je analyzovaná samostatne ako ordinálny ukazovateľ celkového odborného úsudku o pedagogickej využiteľnosti rozhovoru.

#### 2.5.4 Sekundárne premenné a error-based ukazovatele

Sekundárnu vrstvu premenných tvorili dve navzájom odlišné, ale obsahovo prepojené časti. Prvou sú položky A1 až A9, ktoré predstavujú transcript-level PHQ-9 symptom metadata importované zo self-report vrstvy simulovaného participant profilu. Druhou sú položky S1 a S2, ktoré predstavujú expertný odhad celkovej závažnosti a funkčného dopadu.

Na **deskriptívnej úrovni** budú A1 až A9 reportované oddelene od ľudských ratingov ako transcript-level ukazovatele; pri každej doméne je možné uviesť priemer, smerodajnú odchýlku, medián aj interkvartilové rozpätie podľa potreby. Položky S1 a S2 ostávajú ordinálnymi ľudskými ratingmi a budú deskriptívne sumarizované cez frekvencie, mediány a interkvartilové rozpätia. Na **inferenčnej úrovni** však jadro tejto vetvy netvoria jednotlivé doménové položky A1 až A9 ani samotné ordinálne ratingy S1 a S2, ale odvodené error-based ukazovatele, ktoré už majú jasný empirický význam vzhľadom na seed anchor profil.

Keďže pre každý seed boli vopred definované anchor hodnoty, bolo možné odvodiť aj **chybové ukazovatele** vyjadrujúce mieru zhody rozhovoru s cieľovým profilom. Pri A1 až A9 ide o transcript-level zhodu medzi importovaným PHQ-9 profilom a seed anchorom. Pri S1 a S2 ide o expertne hodnotené severity a functional impact položky; keďže seed anchors aj ľudské ratingy používajú rovnakú `1-5` škálu, error-based ukazovatele sa rátajú priamo bez dodatočnej harmonizácie.

- priemerná absolútna chyba v symptomatických doménach = (|A1 − anchor(A1)| + |A2 − anchor(A2)| + ... + |A9 − anchor(A9)|) / 9,
- chyba odhadu závažnosti = absolútna odchýlka expert ratingu S1 od anchor hodnoty S1,
- chyba odhadu funkčného dopadu = absolútna odchýlka expert ratingu S2 od anchor hodnoty S2.

Nižšia hodnota týchto ukazovateľov znamená vyššiu presnosť zhody rozhovoru s cieľovým seed scenárom. Priemerná absolútna chyba v symptomatických doménach slúži prednostne ako transcript-level fidelity ukazovateľ a bude reportovaná ako priemer a smerodajná odchýlka, prípadne doplnkovo medián a interkvartilové rozpätie. Rovnako budú reportované aj `severity_error` a `impact_error`, ktoré sa v inferenčnej časti používajú ako sekundárne približne intervalové outcome-y. Expertne hodnotené severity a functional impact položky zároveň ostávajú v texte viditeľné aj ako samostatná ordinálna ľudská vrstva. V kombinácii s indexom klinickej vierohodnosti tak možno odlíšiť „vyzerá dobre na povrchu" od „zodpovedá zamýšľanému klinickému obrazu".

Keďže seed `S1_anchor` a `S2_anchor` boli finálne zarovnané na rovnakú `1-5` škálu ako expertné ratingy, je možné popri samostatnom vyhodnotení položiek S1 a S2 reportovať aj ich priamu error-based verziu voči seed kotvám.
...
- **Kompozitné a error-based ukazovatele** (index klinickej vierohodnosti, index defektov, priemerná absolútna chyba v symptomatických doménach, chyba odhadu závažnosti, chyba odhadu funkčného dopadu) sú v deskriptívnej časti reportované primárne cez priemer a smerodajnú odchýlku a doplnkovo môžu mať uvedený aj medián a interkvartilové rozpätie. V inferenčnej časti sa pri nich pýtame na rozdiel v priemernej úrovni medzi experimentálnymi podmienkami.
...
**VO1.** Aké sú frekvenčné rozdelenia, mediány a interkvartilové rozpätia položiek G1 až G5 a aká je priemerná úroveň indexu klinickej vierohodnosti pri AI-generovaných simulovaných klinických interview o depresívnej symptomatike?

**VO2.** Aká je priemerná absolútna chyba transcript-level symptomatických domén A1 až A9 voči seed anchorom a aké sú rozdelenia expertných hodnotení S1 a S2, resp. ich priamych error skóre voči seed kotvám?
...
**VO5.** Je pri indexe klinickej vierohodnosti, indexe defektov a hlavných error-based ukazovateľoch prítomná interakcia medzi prítomnosťou štruktúrujúceho klinického usmernenia a štýlom odpovedania simulovaného pacienta?

**VO6.** Aká je miera zhody medzi odbornými hodnotiteľmi pri posudzovaní kvality interview (ICC pre index klinickej vierohodnosti, index defektov a expertné odhady závažnosti a funkčného dopadu)?
...
**H1.** Priemerná hodnota indexu klinickej vierohodnosti bude vyššia pri interview vytvorených so štruktúrujúcim klinickým usmernením než pri interview vytvorených bez neho; sekundárne budú aj položky G1, G3 a G4 vykazovať posun distribúcie k vyšším kategóriám.

**H2.** Distribúcia odpovedí na položke G2 bude pri interview vytvorených so štruktúrujúcim klinickým usmernením posunutá k vyšším kategóriám než pri interview vytvorených bez tohto usmernenia.

**H3.** Priemerná hodnota indexu defektov bude nižšia pri interview vytvorených so štruktúrujúcim klinickým usmernením než pri interview vytvorených bez neho; sekundárne budú aj položky R1 až R5 vykazovať posun distribúcie k nižším kategóriám.

**H4.** Priemerná absolútna chyba v transcript-level symptomatických doménach A1 až A9 voči seed anchorom bude nižšia pri interview vytvorených so štruktúrujúcim klinickým usmernením než pri interview bez neho; rovnaký smer očakávame sekundárne aj pri priamych error skóre závažnosti a funkčného dopadu.

**H5.** Distribúcia odpovedí na položke G5 bude pri interview vytvorených so štruktúrujúcim klinickým usmernením posunutá k vyšším kategóriám než pri interview vytvorených bez tohto usmernenia.
...
**H6.** Priemerná hodnota indexu klinickej vierohodnosti bude vyššia pri interview s vyváženým alebo rozvinutým štýlom odpovedania simulovaného pacienta (P2, P3) než pri interview so zdržanlivým štýlom odpovedania (P1).
...
**H8.** Priemerná absolútna chyba v transcript-level symptomatických doménach A1 až A9 voči seed anchorom bude nižšia pri rozvinutejšom štýle odpovedania simulovaného pacienta (P3) než pri zdržanlivejšom štýle (P1).

Pri širšie formulovanej VO4 zatiaľ samostatne nehypotetizujeme efekt štýlu odpovedania pre expertne hodnotenú závažnosť, expertne hodnotený funkčný dopad a index defektov, pretože pre takto špecifický smer účinku nemáme rovnako silnú literárnu oporu ako pre priemernú absolútnu chybu v transcript-level symptomatických doménach. Tieto výstupy preto budú vo Výsledkoch spracované ako inferenčná odpoveď na VO4, nie ako samostatné smerové hypotézy.
...
**Interrater reliabilita** bola vyhodnocovaná prostredníctvom **intraclass correlation coefficient (ICC)** pre agregované ukazovatele indexu klinickej vierohodnosti, indexu defektov a pre expertne hodnotené položky závažnosti a funkčného dopadu. Transcript-level PHQ-9 symptom metadata A1 až A9 do ICC bloku nevstupujú, keďže nejde o opakované ľudské ratingy toho istého objektu, ale o vlastnosť samotného transkriptu. Primárne bude reportovaný **ICC2k**, teda dvojcestný random-effects model pre average measures a absolútnu zhodu; pri malom alebo nevyváženom paneli však bude tento blok interpretovaný opatrne a vždy spolu s deskriptívnou zhodou jadrovej dvojice hodnotiteľov [@mchugh2012InterraterReliabilityKappa; @nelson2015manyratersordinal]. Ak pre niektorý outcome nebude transcript × rater matica dostatočne plná na reportovateľný ICC odhad, outcome ostane v tejto vrstve len ako pilotná meracia informácia a nebude použitý ako jediný nosný argument kvality merania.
...
Vo všeobecnej špecifikácii modelov bol ako fixný efekt zaradený hlavný účinok prítomnosti štruktúrujúceho klinického usmernenia, hlavný účinok štýlu odpovedania simulovaného pacienta a ich interakcia; referenčnou úrovňou bola kombinácia **bez štruktúrujúceho klinického usmernenia a profil P1**. Pri rater-level modeloch sa ako náhodné efekty zohľadňovali seed scenár a identita hodnotiteľa. Pri transcript-level error vetve (`symptom_error_mean`, `severity_error`, `impact_error`) bol jednotkou analýzy transkript, a preto sa používal random intercept len pre seed scenár.

Tento model je použitý pre premenné:

- index klinickej vierohodnosti (H1, H6, časť H9),
- index defektov (H3, časť H9),
- expertne hodnotená závažnosť ako ordinálny outcome a jej transcript-level error ukazovateľ (H4, časť H9),
- expertne hodnotený funkčný dopad ako ordinálny outcome a jeho transcript-level error ukazovateľ (H4, časť H9).
...
Z hľadiska väzby na výskumné otázky a hypotézy platí nasledovné: modely pre index klinickej vierohodnosti, G2 a G5 odpovedajú na jadro VO1 a zároveň testujú H1, H2 a H5; transcript-level symptom fidelity odvodená z A1 až A9 poskytuje primárnu kvantitatívnu oporu pre H4 a H8 na úrovni samotných transkriptov; expertne hodnotené severity a functional impact položky dopĺňajú VO2 a VO4 ako samostatná ľudská vrstva; hlavné efekty štýlu odpovedania simulovaného pacienta nad indexom klinickej vierohodnosti, G2, G5, expertne hodnotenou závažnosťou, expertne hodnoteným funkčným dopadom a indexom defektov odpovedajú na VO3 a VO4 a pokrývajú H6 až H8; interakčné členy oboch faktorov odpovedajú na VO5 a testujú H9. VO6 je riešená samostatným blokom ICC a VO7 až VO8 samostatným exploračným blokom.
